...
public class RegisterServletE extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("employerAccount") == null) {
			String requestUri = request.getRequestURI();
			request.setAttribute("requestUri", requestUri);
			request.getRequestDispatcher("RegisterForEmployer.jsp").forward(request, response);
			System.out.println("vvvvvvvvvvvv");
			return;
		}
		
		userName = request.getParameter("employerName");
		account = request.getParameter("employerAccount");
...
//		
		...
		if (userManager.getCheckString2().equals("l")) {
		    ...
		    response.sendRedirect("RegisterServletE");
		    return;
		}
