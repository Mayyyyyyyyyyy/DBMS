...
<body>
	...
	<div class="container">
		...
		<c:if test="${not empty jobRecommendList}">
			<c:forEach var="job" items="${jobRecommendList}">
				<div class="post">
					<div class="result">
						<span style="display: none;">${job.jID}</span> ${job.jobName}<br>${job.jContent}
					</div>
					<button class="btn-message" data-action="message"
						data-cid="${job.cID}" onclick="message(this)">私訊</button>
				</div>
			</c:forEach>
		</c:if>
